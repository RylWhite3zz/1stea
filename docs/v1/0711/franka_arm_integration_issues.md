...
### 4.5 Allegro 的中央 probe 工具问题已从 `poke/slide` 路径移除

当前实现已经选择了原先建议中的 fingertip 路线：Allegro `poke/slide` 都只使用
`ff_tip_fingertip_collision + ff_tip_touch`；material scene 的中央 probe 透明、禁碰，
`wp=0` 且不提供任何 slide 信号。因此接入裸 `Franka + Allegro` 时，不再需要虚构
flange 侧置 probe 或 tool changer。reference backend 的左指腹 pad 只是另一套回归工具，
不属于 Franka+Allegro 硬件声明。

这只消除了“工具从哪里来”的问题，并没有让实机触觉自动成立。仍需明确：

1. 仿真的 `ff_tip_touch` 是 MuJoCo 标量 touch，真实 Allegro 指尖必须配置实际触觉传感器，
   或定义经过验证的关节力矩/电流接触估计；二者不能共用同一 sensor profile；
2. `T_flange_palm` 与 `T_palm_ff_tip` 的外参误差会直接变成表面定位和法向误差；
3. wrist F/T 要做 hand+adapter payload、重力和姿态相关 bias 补偿；
4. surface normal/tangent 必须从 world/carriage 固定轴改为目标表面 frame；
5. 机械臂 Cartesian compliance、加减速和结构振动不能混入对象摩擦/刚度 feature。
...
当前 Allegro slide 已使用食指指腹：`ff_tip_touch` 维持法向 preload，baseline-corrected
wrist F/T 提供切向力，wrist-x 做 20 mm 往返。迁移到机械臂后必须把这些 carriage
控制量改写为 surface-frame Cartesian 任务。机械臂后端必须：
...
当前 `sim.allegro_ff_tip_touch+wrist_ft.v1` 只标识 MuJoCo carriage 实现。Franka 仿真或
实机的 arm compliance、F/T 补偿和触觉硬件不同，必须分配新的 sensor/executor profile，
重新标定，不能复用现有 material calibration key。
